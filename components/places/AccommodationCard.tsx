  ...
  return (
    <article className="border-charcoal/10 bg-cream group flex flex-col overflow-hidden rounded-2xl border shadow-[0_1px_2px_rgba(28,28,30,0.04)] transition-shadow hover:shadow-[0_4px_16px_rgba(28,28,30,0.08)]">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <Link href={`/places/${item.slug}`} className="hover:text-ocean">
              <h3 className="text-charcoal font-[family-name:var(--font-display)] text-lg leading-tight font-semibold">
                {item.name}
              </h3>
            </Link>
          ...
          {item.verified_at && (
            <Badge variant="verified">Verified {formatVerificationDate(item.verified_at)}</Badge>
          )}
        </div>
        ...
        <p className="text-charcoal/60 mt-auto font-[family-name:var(--font-mono)] text-xs">
